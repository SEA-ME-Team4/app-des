...
class ToApplicationStubAdapter
 ...
 public:
    /**
    * Sends a broadcast event for Error. Should not be called directly.
    * Instead, the "fire<broadcastName>Event" methods of the stub should be used.
    */
    virtual void fireErrorEvent(const std::string &_Error) = 0;
...
public:
    ...
    /// Sends a broadcast event for Error.
    virtual void fireErrorEvent(const std::string &_Error) {
        auto stubAdapter = CommonAPI::Stub<ToApplicationStubAdapter, ToApplicationStubRemoteEvent>::stubAdapter_.lock();
        if (stubAdapter)
            stubAdapter->fireErrorEvent(_Error);
    }
